...
const daiAddress = "0xEAFFD40B5c50aF6373F46632C1B13BB537b5b7B8";
const daiAbi = [
...
function ContractTokenTransfer() {
  ...
  useEffect(() => {
    ...
    eva.replace();
  }, [transferToken, accountAddress, transferring]);
  ...
  const handleTransfer = async () => {
    ...
    try {
      const dai = ethers.utils.parseUnits(transferTokenAmount, 6);
      const data = await daiWithSigner.transfer(transferAddr, dai);
      ...
      setSuccessMessage(`Transaction Hash ${txHash}`);
    } catch (e) {
      console.log(e);
  ...
  return <div className="h-screen flex flex-col justify-center items-center">
    ...
    {(accountAddress && !transferToken) && (<div className='flex flex-col'>
      <div>
      ...
      <div>
        ...
        <div className="text-2xl font-bold text-green-500 mb-10 bg-slate-700 rounded-lg px-4 py-2">{userBalance} {symbol}</div>
      </div>

      <button onClick={() => setTransferToken(true)} className="self-end hover:scale-110 transition-all bg-sky-600 uppercase font-semibold text-white pl-4 rounded-md flex justify-between w-1/3">
        <span className='p-4'>Transfer {symbol}</span>
    ...
    {(accountAddress && transferToken) && (<div className='flex flex-col w-2/3'>
      <div>
        <div className="text-lg font-bold text-green-600 mb-1">Recepient Address</div>
      ...
      <div>
        <div className="text-lg font-bold text-green-600 mb-1">{symbol} to Transfer</div>
        <input value={transferTokenAmount} onChange={(e) => setTransferTokenAmount(e.target.value)} type="number" className="border-2 w-full text-2xl font-bold text-green-500 mb-10 bg-slate-700 rounded-lg px-4 py-2" />
      </div>
...
export default ContractTokenTransfer;